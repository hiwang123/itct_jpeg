...
void read_sof0(void){
	...
	for(int i=1;i<=sof0.Nf;i++){
		...
		read_uint8_t(&hv);
		sof0.comp[c].H = hv >> 4;
		sof0.comp[c].V = hv & 0xF;
		sof0.Hmax = max(sof0.Hmax, sof0.comp[c].H);
		sof0.Vmax = max(sof0.Vmax, sof0.comp[c].V);
		read_uint8_t(&sof0.comp[c].Tq); //Y: 0, Cr: 1, Cb: 1
	}
}
void read_dht(void){
	uint16_t len;
...
void read_sos(void){
	...
	for(int i=0; i<sos.Ns; i++){
		...
		uint8_t TdTa;
		read_uint8_t(&TdTa);
		sos.comp[Cs].Td = TdTa>>4; //Y: 0, Cr: 1, Cb: 1
		sos.comp[Cs].Ta = TdTa&0xF; //Y: 0, Cr: 1, Cb: 1
	}
	uint8_t ss, se, alah;
	read_uint8_t(&ss);
	read_uint8_t(&se);
