...
void output_bmp(const char *filename){
	...
	for(int i=bmp.V-1;i>=0; i--){
		...
		for(int j=0;j<bmp.H; j++){
			 ...
			 // #1 coef
			 uint8_t R = max(min(round(Y + 1.13983 * (Cr-128)),255),0);
             uint8_t G = max(min(round(Y - 0.39465 * (Cb-128) - 0.58060 * (Cr-128)),255),0);
             uint8_t B = max(min(round(Y + 2.03211 * (Cb-128)),255),0);
             // #2 coef (tsan zhi...)
             //uint8_t R = max(min(round(Y + 1.402 * (Cr-128)),255),0);
             //uint8_t G = max(min(round(Y - 0.344 * (Cb-128) - 0.714 * (Cr-128)),255),0);
             //uint8_t B = max(min(round(Y + 1.772 * (Cb-128)),255),0);
